 ...
 * 3. Backend: Init Privacy Cash SDK with operator keypair (for proof generation)
 * 4. Backend: Generate ZK proof for user's address
 * 5. Backend: Create UNSIGNED transaction with USER as fee payer ← Key!
 ...
 * 9. Backend: Submit signed transaction to blockchain (USER PAYS FEES)
 ...
 * - Operator wallet balance not affected
 */
export async function executeUserPaysDeposit(
  ...
  try {
    ...
    if (!submitResponse.ok) {
      ...
      throw new Error(
        errorData.details || 
        `Submit error: ${submitResponse.status}`
      )
    }

    const submitData = await submitResponse.json()
    const transactionSignature = submitData.transactionHash || submitData.tx
